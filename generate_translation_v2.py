...
Two-column layout, proper section breaks, embedded figures.
"""
...
# ============================================================
# EMU constants matching the reference document exactly
# ============================================================
# Font sizes (EMU)
SIZE_COVER_TITLE = 330200   # 26pt - cover page title
SIZE_TITLE = 177800         # 14pt - paper title (Heading 1)
SIZE_AUTHOR = 177800        # 14pt - author name
SIZE_AFFILIATION = 114300   # 9pt  - affiliation
SIZE_HEADING1 = 177800      # 14pt - section heading
SIZE_HEADING2 = 133350      # 11pt - subsection heading
SIZE_BODY = 120650          # 9.5pt - body text (run default from reference)
SIZE_ABSTRACT = 114300      # 9pt  - abstract/keywords
SIZE_CAPTION = 114300       # 9pt  - figure caption
SIZE_REF = 114300           # 9pt  - references

# Indentation (EMU)
FI_BODY = 266700            # 0.47cm - first line indent for body
RI_BODY = 255905            # 0.45cm - right indent for body paragraphs
...
def set_cell_font(run, east_asia='宋体', name_en='Times New Roman', size=None, bold=False):
    """Set font with eastAsia font for Chinese text."""
...
def set_paragraph_spacing(para, line_spacing=None, line_rule=None):
...
def set_right_indent(para, indent_emu):
    """Set right indentation for a paragraph."""
    pPr = para._element.find(qn('w:pPr'))
    if pPr is None:
        pPr = OxmlElement('w:pPr')
        para._element.insert(0, pPr)
    ...
    twips = int(indent_emu / 635)
    ind.set(qn('w:right'), str(twips))


def set_first_line_indent(para, indent_emu):
...
def add_title(doc, text):
    """Add paper title: Heading 1, center, 14pt 黑体+TNR."""
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run(text)
    set_cell_font(run, '黑体', 'Times New Roman', SIZE_TITLE)
    return p


def add_author(doc, text):
    """Add author: center, 14pt 宋体 bold."""
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run(text)
    set_cell_font(run, '宋体', '宋体', SIZE_AUTHOR, bold=True)
    return p


def add_affiliation(doc, text):
    """Add affiliation: center, 9pt 宋体."""
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run(text)
    set_cell_font(run, '宋体', 'Times New Roman', SIZE_AFFILIATION)
    return p


def add_abstract(doc, text):
    """Add abstract with proper formatting: 9pt, left/right indents."""
    p = doc.add_paragraph()
    set_first_line_indent(p, 0)
    set_right_indent(p, RI_BODY)
    ...
    if ind is None:
        ind = OxmlElement('w:ind')
        pPr.append(ind)
    twips_lr = int(230505 / 635)
    ind.set(qn('w:left'), str(twips_lr))
    ind.set(qn('w:right'), str(twips_lr))

    label = p.add_run('摘  要：')
    set_cell_font(label, '黑体', 'Times New Roman', SIZE_ABSTRACT, bold=True)
    content = p.add_run(text)
    set_cell_font(content, '宋体', 'Times New Roman', SIZE_ABSTRACT)
    return p
...
def add_heading_1(doc, text):
    """Add Heading 1: 14pt, no bold (matching reference exactly)."""
    p = doc.add_paragraph()
    run = p.add_run(text)
    set_cell_font(run, '黑体', 'Times New Roman', SIZE_HEADING1, bold=False)
    set_paragraph_spacing(p, 240, 'auto')  # single spacing
    return p


def add_heading_2(doc, text):
    """Add Heading 2: 11pt 黑体+TNR, no bold, single line spacing."""
    p = doc.add_paragraph()
    run = p.add_run(text)
    set_cell_font(run, '黑体', 'Times New Roman', SIZE_HEADING2, bold=False)
    set_paragraph_spacing(p, 240, 'auto')  # single spacing
    return p


def add_body(doc, text):
    """Add body paragraph: 9.5pt 宋体+TNR, first-line indent, right indent."""
    p = doc.add_paragraph()
    set_first_line_indent(p, FI_BODY)
    set_right_indent(p, RI_BODY)
    run = p.add_run(text)
    set_cell_font(run, '宋体', 'Times New Roman', SIZE_BODY)
    return p


def add_figure_caption(doc, text):
    """Add centered figure caption: 9pt 黑体 bold."""
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run(text)
    set_cell_font(run, '黑体', 'Times New Roman', SIZE_CAPTION, bold=True)
    return p


def add_figure(doc, img_path, width_inches=3.2):
    """Add an image centered in the column.
    For two-column layout, max usable width per column is ~3.2 inches.
    For full-width (single column sections), can use up to 7 inches.
    """
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run()
    run.add_picture(img_path, width=Cm(width_inches * 2.54))
    return p


def add_section_break(doc, section_type='continuous'):
    """Add a section break."""
    new_section = doc.add_section()
    if section_type == 'continuous':
        new_section.start_type = 2  # WD_SECTION.CONTINUOUS
    elif section_type == 'new_page':
        new_section.start_type = 1  # WD_SECTION.NEW_PAGE
    # Set page size and margins
    new_section.page_width = Emu(PAGE_W * 635)
    new_section.page_height = Emu(PAGE_H * 635)
    new_section.top_margin = Emu(MARGIN_TB * 635)
    new_section.bottom_margin = Emu(MARGIN_TB * 635)
    new_section.left_margin = Emu(MARGIN_LR * 635)
    new_section.right_margin = Emu(MARGIN_LR * 635)
    return new_section


def set_columns(section, num_cols, space=COL_SPACE):
...
def add_ref_para(doc, text):
    """Add a reference paragraph: 9pt TNR."""
    p = doc.add_paragraph()
    run = p.add_run(text)
    set_cell_font(run, '宋体', 'Times New Roman', SIZE_REF)
    return p
...
# ----- SECTION 0: Cover page (single column) -----
section0 = doc.sections[0]
section0.page_width = Emu(PAGE_W * 635)
section0.page_height = Emu(PAGE_H * 635)
section0.top_margin = Emu(MARGIN_TB * 635)
section0.bottom_margin = Emu(MARGIN_TB * 635)
section0.left_margin = Emu(MARGIN_LR * 635)
section0.right_margin = Emu(MARGIN_LR * 635)
# Single column (default)
set_columns(section0, 1)

# Cover title - reference uses 华文行楷
p = doc.add_paragraph()
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
run = p.add_run('TD-MPC2：面向连续控制的可扩展鲁棒世界模型')
set_cell_font(run, '华文行楷', 'Times New Roman', SIZE_COVER_TITLE)

# Blank line
doc.add_paragraph()

# Author on cover - reference uses 楷体_GB2312
p = doc.add_paragraph()
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
run = p.add_run('Nicklas Hansen    Hao Su    Xiaolong Wang')
set_cell_font(run, '楷体_GB2312', 'Times New Roman', SIZE_AUTHOR)

# Affiliation on cover - reference uses 楷体_GB2312
p = doc.add_paragraph()
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
run = p.add_run('（加州大学圣地亚哥分校，美国 加利福尼亚州 圣地亚哥 92093）')
set_cell_font(run, '楷体_GB2312', 'Times New Roman', SIZE_AFFILIATION)

# ----- SECTION 1: Two-column body (continuous break) -----
section1 = add_section_break(doc, 'continuous')
set_columns(section1, 2, COL_SPACE)

# Title (again, in two-column section)
...
# Figure 1: Overview
fig_dir = r'E:\Code\Embed\作业\智能机器人控制\figures'
add_figure(doc, os.path.join(fig_dir, 'fig1_overview.png'), 3.2)
add_figure_caption(doc, '图1 TD-MPC2与现有方法的比较。在104个连续控制任务上，使用单一超参数集即可取得优于基线的性能（左）。'
...
# Figure 2: Tasks
add_figure(doc, os.path.join(fig_dir, 'fig2_tasks.png'), 3.2)
add_figure_caption(doc, '图2 TD-MPC2在104个不同任务上的表现。任务涵盖DMControl（30个任务）、'
...
add_figure(doc, os.path.join(fig_dir, 'fig3_architecture.png'), 3.0)
add_figure_caption(doc, '图3 TD-MPC2架构。观测s被编码为其潜在（归一化）表示z。'
...
# Figure 4: Single-task RL
add_figure(doc, os.path.join(fig_dir, 'fig4_single_task.png'), 3.2)
add_figure_caption(doc, '图4 单任务强化学习。在DMControl、Meta-World、ManiSkill2和MyoSuite上的'
...
# Figure 5: High-dimensional locomotion
add_figure(doc, os.path.join(fig_dir, 'fig5_locomotion.png'), 3.2)
add_figure_caption(doc, '图5 高维运动。在Humanoid（A∈R^17）和Dog（A∈R^28）上的'
    '回合回报。TD-MPC2显著优于SAC和DreamerV3。')

# Figure 6: Object manipulation
add_figure(doc, os.path.join(fig_dir, 'fig6_manipulation.png'), 3.2)
add_figure_caption(doc, '图6 物体操作。在5个Meta-World任务上的成功率。'
...
# Figure 7: Scaling
add_figure(doc, os.path.join(fig_dir, 'fig7_scaling.png'), 3.2)
add_figure_caption(doc, '图7 多任务模型扩展。归一化分数随模型参数量增大而持续提升，'
    '在两个任务集上均未出现饱和迹象。')

# Figure 8: Multi-task
add_figure(doc, os.path.join(fig_dir, 'fig8_multitask.png'), 3.2)
add_figure_caption(doc, '图8 多任务性能。不同规模的多任务模型在各个任务领域上的性能对比。')
...
# Figure: Few-shot
add_figure(doc, os.path.join(fig_dir, 'fig_fewshot.png'), 3.2)
add_figure_caption(doc, '图9 少样本学习。预训练世界模型微调用于未见任务的效果。'
    '在低数据量条件下（2万环境步），TD-MPC2相比从零开始学习提升2倍。')
...
# Figure: Task embeddings
add_figure(doc, os.path.join(fig_dir, 'fig_embeddings.png'), 3.2)
add_figure_caption(doc, '图10 任务嵌入可视化。语义上相似的任务在嵌入空间中距离较近，'
    '嵌入相似性与任务动力学而非目标对齐。')

# Figure 9: Ablations
add_figure(doc, os.path.join(fig_dir, 'fig9_ablations.png'), 3.2)
add_figure_caption(doc, '图11 消融实验。各设计选择对TD-MPC2鲁棒性和性能的贡献。'
...
# Figure 10: Visual RL
add_figure(doc, os.path.join(fig_dir, 'fig10_visual_rl.png'), 3.2)
add_figure_caption(doc, '图12 视觉强化学习。在10个图像输入DMControl任务上的回合回报。'
...
# ===== 参考文献 (single column) =====
# In reference, section break is embedded in a Heading 1 paragraph
add_heading_1(doc, '参考文献')
# Add section break to the last paragraph (the heading)
last_para = doc.paragraphs[-1]
pPr = last_para._element.find(qn('w:pPr'))
if pPr is None:
    pPr = OxmlElement('w:pPr')
    last_para._element.insert(0, pPr)
...
styp = OxmlElement('w:type')
styp.set(qn('w:val'), 'continuous')
sectPr.append(styp)
pPr.append(sectPr)
...
# ===== 英文版本 (single column) =====
# Add section break embedded in a Heading 1 paragraph
p = doc.add_paragraph()
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
run = p.add_run('TD-MPC2: Scalable, Robust World Models for Continuous Control')
set_cell_font(run, 'Times New Roman', 'Times New Roman', 152400)  # 12pt
# Embed section break in this paragraph
pPr = p._element.find(qn('w:pPr'))
if pPr is None:
    pPr = OxmlElement('w:pPr')
    p._element.insert(0, pPr)
...
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
run = p.add_run('Nicklas Hansen    Hao Su    Xiaolong Wang')
set_cell_font(run, 'Times New Roman', 'Times New Roman', SIZE_AUTHOR, bold=True)

p = doc.add_paragraph()
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
run = p.add_run('(University of California San Diego, San Diego, CA 92093, USA)')
set_cell_font(run, 'Times New Roman', 'Times New Roman', SIZE_AFFILIATION)

# English abstract
p = doc.add_paragraph()
set_right_indent(p, RI_BODY)
label = p.add_run('Abstract: ')
set_cell_font(label, 'Times New Roman', 'Times New Roman', SIZE_ABSTRACT, bold=True)
content = p.add_run(
...
set_cell_font(content, 'Times New Roman', 'Times New Roman', SIZE_ABSTRACT)
